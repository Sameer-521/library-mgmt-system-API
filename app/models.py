...
class User(Base):
    ...
    user_uid: Mapped[str] = mapped_column(
        String(50), unique=True, default=generate_user_id
    )
...
class Loan(Base):
    ...
    loan_id: Mapped[str] = mapped_column(
        String(50), unique=True, default=generate_loan_id
    )
    user_uid: Mapped[str] = mapped_column(String, ForeignKey("users.user_uid"))
    bk_copy_barcode: Mapped[str] = mapped_column(
        String, ForeignKey("book_copies.copy_barcode"), index=True
    )
    ...
    due_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True), default=default_loan_due_date
    )
...
class BkCopySchedule(Base):
    ...
    schedule_id: Mapped[str] = mapped_column(
        String(50), nullable=False, default=generate_schedule_id
    )
    status: Mapped[enum.Enum] = mapped_column(
        Enum(ScheduleStatus), default=ScheduleStatus.ACTIVE, index=True
    )
    created_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True), server_default=func.now()
    )
